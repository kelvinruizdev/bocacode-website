...
const SoftwareEngineering = () => {
  ...
  return (
    <main className={styles.main}>
      ...
      <div className='w-70 maxwidth-1366 bg-lila rounded-8 p-32 row gap-45 m-auto'>
      ...
      <div className='py-50 maxwidth-1366 m-auto'>
        <div className='text-center py-30 w-60 m-auto column gap-8'>
          <h4 className='fs-16 fw-600 lh-24'>{data?.what_learn?.sub_title}</h4>
          <h2 className='fs-30 fw-800 lh-36'>{data?.what_learn?.title}</h2>
          <p className='fs-18 fw-400 lh-24 color-gray'>{data?.what_learn?.text}</p>
        </div>
        <div className='grid grid-col-3 w-60 m-auto gap-24'>
          {data?.what_learn?.items.map(item =>
            <div className='row gap-8 color-gray'>
              <Icon icon={data?.what_learn?.icon} width="24px" />
              <p>{item}</p>
            </div>)}
        </div>
      </div>
      ...
      {data?.qualities && (
        <div className='row gap-45 wrap py-30 justify-center maxwidth-1366 m-auto justify-beetwen'>
